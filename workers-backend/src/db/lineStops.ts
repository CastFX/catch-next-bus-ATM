...
export const lineStopsSQL = sqliteTable("lineStops", {
  ...
  minutesFromHome: text("minutesFromHome", { mode: "json" })
    .notNull()
    .$type<WalkTime[]>(),
...
export const setLineEstimates = async (
  ...
  await db
    .update(lineStopsSQL)
    .set({ waitMessage, estimates, lastUpdatedAt })
    .where(
      and(eq(lineStopsSQL.lineId, lineId), eq(lineStopsSQL.stopCode, stopCode))
    );
};
